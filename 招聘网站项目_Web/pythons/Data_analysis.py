...
from pythons import Data_cleaning as dc
import pandas as pd
import time
import os
...
def general_func(job, city, filename, num, path='../database/'):
    job_name = []
    company_name = []
    work_city = []
    company_type = []
    ...
    for i in range(1, (int(num) + 1)):
        ...
        job_name, work_city, company_name, company_type, company_size, company_ind, date, education = rc.list_fulfilling(
            job_info_list, job_name, company_name, company_type,
            date, company_ind, company_size,
            salary_level, work_city, education)
        t2 = time.time()
        if (t2 - t1) < 0.1:
            print("第" + str(i) + "页为空！")
        else:
    ...
    dc.store_plot(csv, job) # 薪资箱线图
    dc.education_plot(csv, job) # 学历条形图
    dc.format_bubble_plot(csv, job) # 泡泡图
    dc.format_dir_wordcloud(csv, job) # 公司经营方向词云图
    return final_list
